...
//! `MultiControlMode`: the exact CCX/CSwap subspace kernel vs the 15-gate
//! decomposition, both against the **CPU f64 statevector**.
//!
...
//! the `[0,2,1,3]` CUDA basis permutation, the slot layout), so they would agree
//! on a shared misreading. This repository has already shipped that exact
...
//! gated against Qiskit by `omega-xcheck`, whose corpus now contains `ccx` and
//! `cswap` for this reason. The chain is CUDA → CPU → Qiskit, with no link
...
fn both_modes_match_the_cpu_statevector_on_ccx_and_cswap() {
    let Ok(base) = CudaStatevectorBackend::new() else {
        eprintln!("SKIP (NOT A PASS): no CUDA device");
        return;
    };
    let exact = CudaStatevectorBackend::new()
        .expect("cuda")
