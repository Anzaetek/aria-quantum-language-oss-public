//! `Reset` must implement the reset CHANNEL on the MPS backend too.
//!
...
//! derivation. Aer's `matrix_product_state` method agrees with its statevector
//! and stabilizer methods on all three.
...
fn ones(m: &HashMap<omega_core::outcome::Outcome, u32>, bit: u64) -> u32 {
    // `bit` is a MASK in the old u64 convention; read the corresponding index.
...
/// A — partner is maximally mixed in Z (post-selection would fail this).
#[test]
fn reset_leaves_partner_mixed_in_z() {
    let k = ones(&counts(&bell_reset0(), 7), 0b10);
    assert!(
        k.abs_diff(SHOTS / 2) < BAND,
        "mps: got {k}/{SHOTS} ones on q1, want ~{}",
        SHOTS / 2
    );
}
...
fn reset_leaves_partner_mixed_in_x() {
    ...
    assert!(
        k.abs_diff(SHOTS / 2) < BAND,
        "mps: got {k}/{SHOTS} ones on q1 after H, want ~{}",
        SHOTS / 2
    );
}

/// C — reset of |−⟩ must read 0 with certainty (the fold zeroed the chain).
#[test]
fn reset_of_minus_state_reads_zero() {
    ...
    c.add_op(g(GateKind::Reset, &[0]));
    let m = counts(&c, 7);
    assert_eq!(ones(&m, 0b1), 0, "mps: reset(|−⟩) must be |0⟩, got {m:?}");
}
...
fn reset_qubit_is_zero_on_every_shot() {
    ...
    assert_eq!(ones(&m, 0b1), 0, "mps: q0 must be |0⟩, got {m:?}");
}
