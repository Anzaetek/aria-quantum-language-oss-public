//! CUDA accelerator for the Pauli-propagation **branch** step.
//!
...
//! parallel, so we offload it to the GPU: pack each term's symplectic `(x, z)`
//! into u64 words (SoA), launch one NVRTC kernel that emits the (≤2) children per
//! term, then merge the children back into a `PauliSum` on the host.
//!
//! [`cuda_branch`] has the exact signature of `omega_backend_pauliprop::BranchHook`,
//! so it drops into `PauliPropBackend::with_branch_hook`. It returns `false`
//! (leaving the sum untouched, so the caller runs the identical CPU path) when
//! the `cuda` feature is off, the host isn't Linux/Windows, no CUDA driver is
//! present, or the term count is below the GPU-worthwhile threshold — the same
//! transparent-fallback contract as `omega-backend-mps-cuda`.
...
/// that want to force the GPU path on a small sum).
...
/// `PauliPropBackend::with_branch_hook(cuda_branch)`.
