//! GPU shot-sampling correctness — `OpenClState::sample_shots_gpu`.
//!
//! Mirror of `omega-backend-statevector-metal/tests/shot_sampling_tvd.rs`.
//! The OpenCL kernel chain is a 1:1 port of Metal's three-stage
//! pipeline (probabilities → Hillis-Steele inclusive scan → Philox-
//! driven CDF binary search), so the same acceptance gate applies:
//! empirical TVD ≤ 1e-2 at 10⁵ shots, given the pure-statistical
//! floor of `1/√N ≈ 3e-3` at this shot count. Determinism is pinned
//! separately so a same-seeded sample returns byte-identical counts.
...
fn tvd(counts: &HashMap<u64, u32>, probs: &[f64]) -> f64 {
    ...
    // out-of-range slot as a TVD contribution.
...
fn write_uniform_superposition(
    state: &mut omega_backend_statevector_opencl::OpenClState,
    k: usize,
) {
