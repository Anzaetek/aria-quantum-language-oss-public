...
* **The in-repo Reck tests are circular.** `random_unitary` (`decompose.rs:137`)
  builds its test matrices from `build_unitary` of the *same* beam-splitter
  primitives used to reconstruct, so a convention change in
  `apply_beam_splitter_rx` cancels on both sides and the test cannot see it.
  The external check above is what actually establishes correctness; the fix is
  to generate test unitaries **independently** (Haar via QR, or hard-coded DFT).
