...
* **Preparations are constructors, not operators.** `squeezed_vacuum(r)` and
  `coherent(α)` build a state *from vacuum*; there is no `displace(&mut self,…)`.
  So an ordinary CV circuit like *squeeze then displace* cannot be written. The
  cross-check reports such a case as `NOT COMPARED` rather than dropping it.
...
`omega-backend-photonics` has `reck_decompose`, so this is reachable. But
**`clements_decompose` currently delegates to Reck** (`decompose.rs:126`) while
the module header advertises Clements' loss tolerance. A QFT example built on it
would inherit a claim the code does not implement, so that is fixed or renamed
first. Tracked in `FIXES_PLAN.md` I4b.
